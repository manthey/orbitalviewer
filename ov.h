...
VOID CALLBACK timer              (HWND hwnd,ulong msg,ulong id,long time);
         ...
         camera_figure_draw(long w, long h, long scan, uchar *buf,
                         DATA *data, double *phys);
...
HPALETTE make_windows_palette(uchar *pal, long bgr);
         mouse          (HWND hwnd, long but, long up, long flag, long x,
                         long y);
...
void     render_physical(double *val, double *dlt, double *phys,
                         lmatrix *ang);
         render_triangle_texture(long w, long h, long scan, uchar *scrbuf,
...
extern char *DispInfo[], *DispMode[], *DistText[], HelpFile[], lastview[],
...
extern long AVIHeader[], AVILoad, BitsPixel, Busy, CloseMode, Down, EffScale,
       LastMove, LockList[], NumLocked, NumWindows, Perspect, PreviewDelay,
